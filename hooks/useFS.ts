...
import { FSState, FileState, PERMISSION, FSStore } from "../types";
  ...
  cd: (pathToDir: string) =>
    set((state: FSState) => {
      ...
      while (pathArr.length > 0) {
        ...
        const matchedDir = tempDir.children.find(
          ({ name }: FileState) => name === popedDir
        );
